...
df1 <- findboxes(coords_high_ecol,
  ...
  point_padding_y = Reduce("-", rev(range(point_data$lat))) * .5,
...
bar_charts_offset + gg_legend + 
  plot_layout(design = layout)

```

Trying example from above but choosing an offset for long
...
Trying manual offset put through the geom_boxes function
...
Making large plot to go in supplemental materials
...
plot_patch <- wrap_plots(plot_list, ncol = 1, height = 0.05)
...
# using patchwork to join together
# plot_patch <- wrap_plots(plot_list_all, ncol = 1, height = 0.05)

wrap_plots(plot_list_all[1])
...
# making a function that takes the lat/long limits and tidys data, makes barcharts, and assembles map

```{r}

supplement_figures <- function(x1, x2, y1, y2){
  base_map_zoom <- ggplot(coords) +
  geom_sf(data = us_boundaries()) +
  geom_sf(data = canada) + 
  geom_point(aes(long, lat), color = "black")+
  coord_sf(xlim = c(x1, x2), ylim = c(y1, y2))
  
  bar_data_zoom <- bar_data_all %>% 
  filter(long >= x1 & long <= x2) %>% 
  filter(lat >= y1 & lat <= y2)
  
  data_len <- length(bar_data_zoom)
  
  plot_list_zoom <- vector("list", length = 66)
  ...
  for( i in 1:data_len){
    ...
    plot_list_zoom[[i]] <- test_pie
  }
  
supp_zoom <- (wrap_plots(plot_list_zoom[1:data_len], ncol = 1)) | base_map_zoom

supp_zoom
  
}

supplement_figures(-123.5, -118.5, 34, 38.5)
...
ggplot() +
  ...
  geom_sf(data = canada) +
  geom_sf(data = mexico) +
  geom_sf(data = centroid_points, aes(color = Ecol1), size = 3) +
...
ggplot() +
  ...
  geom_sf(data = canada) +
  geom_sf(data = mexico) +
  geom_sf(data = SNAPP_estuary_polygons_centroid$centroid, aes(color = SNAPP_estuary_polygons_centroid$Ecol1, size = 2, shape = as.factor(SNAPP_estuary_polygons_centroid$high_ecol))) +
  scale_color_gradientn(colours = c(
    "#eff3ff",
    "#6baed6",
    "#084594"
