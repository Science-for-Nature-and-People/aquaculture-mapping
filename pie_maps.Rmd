...
#Adding a selector box to leaflet: unfortunately, the minichart function does not support the group argument
...
ggplot(station.points) +
  ...
  geom_sf(shape = NA, color = stations)+
  station.annotations
...
bar_data <-  data_scores %>%
  drop_na("long") %>% 
  dplyr::select(-coastwide_coservation) %>% 
...
for( i in 1:10){
  ...
  temp_plot <- ggplot(temp_data, aes(x = "", y = category, fill = score))+
  geom_bar(width = 1, stat = "identity", show.legend = FALSE)
  
  plot_fig <- image_graph(width = 100, height = 75, res = 96)
  
  #final <- image_composite(fig, plot_fig, offset = "+",temp_data$long,"+",temp_data$lat)
  final <- image_composite(fig, plot_fig, offset = "+75+150")
