...
cropped_map  <- map_data("world", xlim = c(-124.5, -117.5), ylim = c(33, 48.5))
...
=======
  dplyr::select(c(estuary_or_subbasin, lat, long, ecological, harvest,  )) %>% 
>>>>>>> cb54084b7a9ed2d301533aecafbfd4ab5fdab98f
...
test_plots <- bar_data %>% 
  ...
  mutate(plot = map2(data, estuary_or_subbasin,
                     ~ggplot(data = .x) +
                       ...
                       geom_bar(width = 5, stat = "identity", show.legend = FALSE) +
<<<<<<< HEAD
...
estuary_annotation <- plot_points %>% 
  ...
  mutate(annotation = pmap(list( X, Y, plot), #creating an annotation column, then using the coordinates and plots in the annotation_custom() function
                           ~annotation_custom(ggplotGrob(..3), # ..3 refers to the third argument, so plot 
                                              xmin = ..1, xmax = ..1 + 5, # ..1 refers to X
                                              ymin = ..2, ymax = ..2 + 5))) %>% # ..2 refers to Y
  pull(annotation)

# make ggplot and add list of notations
ggplot(plot_points) +
  #geom_polygon(data = cropped_map, aes(x = long, y = lat, group = group), fill="lightgray", colour = "white")+
  geom_sf(data = us_boundaries()) +
  geom_sf(data = canada) + 
  xlim(c(-130, -115))+
  ylim(c(30, 53))+
