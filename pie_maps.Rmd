...
df1 <- findboxes(coords_high_ecol,
  xcol = "long", ycol = "lat",
  box_padding_x = Reduce("-", rev(range(coords$lat))) * .5,
  box_padding_y = Reduce("-", rev(range(coords$long))) * .55,
  point_padding_x = Reduce("-", rev(range(coords$long))) * .5,
  point_padding_y = Reduce("-", rev(range(coords$lat))) * .55,
  force = 1e-3, #force = repulsion
  xlim = c(-120, -112),
  ylim = c(27, 55)
)
...
Zoom_point_3 <- ggplot() +
 geom_sf(data = us_boundaries()) +
  ...
  scale_shape_discrete(name = "Ecological Score", labels = c("> 0.5", "< 0.5"))+
  labs(color = "Ecological Score", shape = "Ecology Score") + #changing names on legend
  coord_sf(xlim = c(-120, -112), ylim = c(24, 34.5)) +
  guides(size = FALSE)+ #removing size from legend
  theme_void()
